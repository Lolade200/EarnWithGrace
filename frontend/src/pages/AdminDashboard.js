          ...
          <form onSubmit={handleSurveySubmit} className="survey-form">
            ...
            {questions.map((q, qIndex) => (
              <div key={qIndex} className="question-box">
                <div className="q-header">
                  ...
                  {questions.length > 1 && (
                    <button type="button" className="remove-btn" onClick={() => handleRemoveQuestion(qIndex)}>Remove Question</button>
                  )}
                </div>
                ...
                <div className="options-list">
                  {q.options.map((opt, oIndex) => (
                    <div key={oIndex} className="opt-row">
                      ...
                      />
                      {q.options.length > 2 && (
                        <button type="button" className="remove-btn" onClick={() => handleRemoveOption(qIndex, oIndex)}>
                          <FontAwesomeIcon icon={faXmark} />
                        </button>
                      )}
                    </div>
                  ))}
                  <button type="button" className="secondary-btn" onClick={() => handleAddOption(qIndex)}>
            ...
            <div className="form-actions-row">
              ...
              <button type="submit" className="create-btn" disabled={actionLoading["survey-submit"]}>
                <FontAwesomeIcon icon={actionLoading["survey-submit"] ? faSpinner : faPlus} spin={actionLoading["survey-submit"]} /> Publish Survey
              </button>
            </div>
          </form>
        </section>
